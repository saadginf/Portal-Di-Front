...
export const getFieldsDoc = ()=> dispatch  =>{
   
    dispatch({
        type: action.FETCH_FIELDSDOC_START
    })
    Axios.get('http://localhost:8080/api/archive/fieldsDoc')
    .then(res=>{
       console.log(res.data) 
    
       dispatch({
        type: action.FETCH_FIELDSDOC_SUCCESS,
        payload: res.data
    })
    })
    .catch(err=>{
        console.log(err)
        dispatch({
            type: action.FETCH_FIELDSDOC_FAIL,
            payload: err.response.data
        })
    }).finally(()=>{
        dispatch({
            type: action.FETCH_FIELDSDOC_END,
